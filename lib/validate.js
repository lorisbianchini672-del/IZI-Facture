...
function refine(definition, predicate, options = {}) {
  ...
  return {
    ...
    parse(value, currentPath, issues) {
      ...
      if (!predicate(parsed)) {
        // Le chemin de l'issue ne doit jamais commencer par un point :
        // « due:date_order » à la racine, « facture.due:date_order » sinon.
        const issuePath = path
          ? (currentPath ? `${currentPath}.${path}` : path)
          : currentPath;
        issues.push(issue(issuePath, code, message));
      }
      return parsed;
    }
  };
  ...
  return schema('object', (value, path, issues) => {
    ...
    if (strict) {
      for (const key of Object.keys(source)) {
        // hasOwnProperty et non `in` : `in` remonte Object.prototype, donc
        // `__proto__` et `constructor` y sont toujours « présents » et une
        // tentative de pollution passerait silencieusement.
